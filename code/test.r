      ...
      column(8,
        fluidRow(
          ...
          if(dayParams$runType == "luminance")
            ...
            column(6, numericInput(ns("lum"), "Stimulus Luminance (cd/m2)", value = 400))
        ),
        fluidRow(
...
test <- function(input, output, session) {
  ...
  listen     <- reactiveTimer(500) # every 200 ms
...
fillOpiParams <- function(serverIP) {
  ...
  if(dayParams$machine == "Daydream") {
    ...
    opiParams$port <- 50008
    opiParams$lut  <- read.csv(paste0("../config/", dayParams$gammaFile), header = FALSE)$V1
